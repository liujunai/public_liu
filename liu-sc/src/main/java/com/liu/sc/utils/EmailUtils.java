...
public class EmailUtils
{
     ...
     * @param email
     */
    public static void send(Session session, final Email email) throws MessagingException,
            IOException
    {

        MimeMessage msg = new MimeMessage(session);// 创建邮件对象
        msg.setFrom(new InternetAddress(email.getFrom()));// 设置发件人
        ...
        // 添加附件
        List<AttachBean> attachBeanList = email.getAttachs();// 获取所有附件
